...
//        $expirationDate = \DateTime::createFromFormat('M d, Y', $params['expirationDate']);
        ...
        if ($billingInfo->getCredit() > $params['credit']){
            ...
            if ($subscription['success']) {
                ...
                if (!$effectiveDate = $billingInfo->getEffectiveDate()) {
                    $effectiveDate = new \DateTime('today');
                }

                if ($params['frequency'] == 0) {
                    $addMonth = 1;
                } elseif ($params['frequency'] == 1) {
                    $addMonth = 12;
                } else {
                    $addMonth = 0;
                }

                $expirationDate = $effectiveDate->add(new DateInterval('P'.$addMonth.'M'));
